...
//       {isSidebarOpen && (
//         <div className='fixed inset-0 bg-gray-900 bg-opacity-75 z-40 md:hidden transition-all duration-300 ease-in-out'>
//           <div className='fixed top-0 left-0 w-2/3 bg-white h-full p-4 transition-all duration-300 ease-in-out'>
...
//               {links.map((link, index) => (
//                 <li key={index} className='mb-4 sm:text-lg hover:bg-green-700 w-full py-2'>
...
// export default Navbar; // Correctly exporting the component
...
//           <ul className='flex flex-col items-start mt-10'>
//             {links.map((link, index) => (
//               <li key={index} className='mb-4 sm:text-lg hover:bg-green-700 hover:text-white w-full py-2'>
//                 <Link href={link} className='block w-full' onClick={() => handleLinkClick(link)}>
//                   {link === '/' ? 'Home' : (link.split('/').pop() || '').replace(/-/g, ' ').replace(/^\w/, (c) => c.toUpperCase())}
//                 </Link>
//               </li>
